...
class SysfunctionController extends ActionController
{
	public function init(){
		...
		// OPTIONS
		$this->_options['tableName']	= 'Admin\Model\SysfunctionTable';
		$this->_options['formName']		= 'formAdminSysfunction';
    ...
    public function statusAction()
    {
    	...
    	if($this->getRequest()->isPost()){
    		...
    		echo '<h3 style="color:red ;font-weight:bold">'.$flagAction.'</h3>';
    ...
    public function showAction()
    {
    	...
    	if($this->getRequest()->isPost()){
    		$flagAction = $this->getTable()->changeShow($this->_params['data'], array('task' => 'change-show'));
    		echo '<h3 style="color:red ;font-weight:bold">'.$flagAction.'</h3>';
    		if($flagAction == true) $this->flashMessenger()->addMessage('Ẩn hiện của phần tử đã được cập nhật thành công!');
    	}
    	$this->goAction();
    }
    ...
    public function menuAction()
    {
    	...
    	if($this->getRequest()->isPost()){
    		$flagAction = $this->getTable()->changeMenu($this->_params['data'], array('task' => 'change-menu'));
    		echo '<h3 style="color:red ;font-weight:bold">'.$flagAction.'</h3>';
    		if($flagAction == true) $this->flashMessenger()->addMessage('Loai menu của phần tử đã được cập nhật thành công!');
    	}
    	$this->goAction();
    }
    ...
    public function formAction()
    {
    	...
    	if(!empty($this->_params['data']['id'])) {
    		...
    		if(!empty($item)){
    			$myForm->setInputFilter(new \Admin\Form\SysfunctionFilter(array('id' => $this->_params['data']['id'])));
    			$myForm->bind($item);
    			$task	= 'edit-item';
    		}
    	}
