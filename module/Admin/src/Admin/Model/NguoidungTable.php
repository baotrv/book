...
class NguoidungTable extends AbstractTableGateway {
		...
		if($options['task'] == 'list-item') {
			...
			$result	= $this->tableGateway->select(function (Select $select) use ($arrParam){
				...
				if(!empty($ssFilter['filter_status'])){
					...
					$select->where->equalTo(TABLE_NGUOIDUNG.'.trang_thai',$status);
				}
				
				if(!empty($ssFilter['filter_group'])){
					$select->where->equalTo(TABLE_NGUOIDUNG.'.nhom_nguoi_dung_id',$ssFilter['filter_group']);
				}
				
				if(!empty($ssFilter['filter_keyword_type']) && !empty($ssFilter['filter_keyword_value'])){
					if($ssFilter['filter_keyword_type'] != 'all') {
						$select->where->like(TABLE_NGUOIDUNG.'.' .$ssFilter['filter_keyword_type'], '%'.$ssFilter['filter_keyword_value'].'%');
					}else{
						$select->where->NEST
									  ->like('ten_nguoi', '%'.$ssFilter['filter_keyword_value'].'%')
									  ->or
		...
		if($options['task'] == 'list-item') {
			...
			$result	= $this->tableGateway->select(function (Select $select) use ($arrParam){
				...
				$select->columns(array(
							'id','ten_nguoi','ngay_sinh','gioi_tinh','cmnd','trang_thai','nghe_nghiep','nhom_nguoi_dung_id'
						))
						->join(
							array('g' => 'group'),
							TABLE_NGUOIDUNG.'.nhom_nguoi_dung_id = g.id',
				...
				if(!empty($ssFilter['filter_keyword_type']) && !empty($ssFilter['filter_keyword_value'])){
					if($ssFilter['filter_keyword_type'] != 'all') {
						...
						$select->where->NEST
									  ->like(TABLE_NGUOIDUNG.'.ten_nguoi', '%'.$ssFilter['filter_keyword_value'].'%')
									  ->or
									  ->equalTo(TABLE_NGUOIDUNG.'.id', $ssFilter['filter_keyword_value'])
									  ->UNNEST;
					}
				}
			});
