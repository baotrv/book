...
class GrantingagenciesTable extends AbstractTableGateway {
	...
	public function parentById($arrParam = null, $options = null){
		...
		if($options['task'] == 'form-grantingagencies') {
			$result	= $this->tableGateway->select(function (Select $select) use ($arrParam){
				$select->columns(array('Id','Parentid'));
		...
		if($options['task'] == 'form-grantingagencies-parnetid') {
			$result	= $this->tableGateway->select(function (Select $select) use ($arrParam){
		...
		if($options['task'] == 'form-grantingagencies-array-parnetid') {
			$result	= $this->tableGateway->select(function (Select $select) use ($arrParam){
				//ARRAY ID POST ARRPARAM
	...
	public function itemInSelectbox($arrParam = null, $options = null){
		...
		if($options['task'] == 'form-grantingagencies') {
			$arrId	= $this->parentById($arrParam,array('task' => 'form-grantingagencies'));
			//GET PARENT BY ID
	...
	public function deleteItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'one-delete') {
			if(!empty($arrParam['parent_id'])){
				...
				$arrId = $this->parentById($arrParam,array('task' => 'form-grantingagencies-parnetid'));
		...
		if($options['task'] == 'multi-delete') {
			if(!empty($arrParam['cid'])){
				...
				$where = new Where();
				//CHECK ID EQUAL CHILDER
				$arrId = $this->parentById($arrParam,array('task' => 'form-grantingagencies-array-parnetid'));
				if (!empty($arrId) && ($arrId->Parentid > 0)){
					return false;
				}else{
